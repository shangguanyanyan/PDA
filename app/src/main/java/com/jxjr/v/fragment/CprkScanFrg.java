...
public class CprkScanFrg extends ABaseBillScanFrg<JrPdaCprkbillentry,CprkActivity> implements IfCprkScanFrgView{
	/**
	...
	protected int setLayoutID() {
		// TODO Auto-generated method stub
		return R.layout.activity_cprk_scan;
	}
	...
	public void popDecodeThBarcode(boolean sucflg, String msg,
		...
		if(sucflg){
			if(itemDTO !=null)	{	
				...
				mActivity.getEntity().getBill().setFcreateuserid(PbF.int2Str(GI.SESSION.getfUserID()));
				mActivity.getEntity().getBill().setFcreateusername(GI.SESSION.getfUserName());
...
//		barcode.setOnTouchListener(AZUIPub.noInputToucher);
	...
	boolean ifDataLoaded(){
		if(entrystatus==0){			
			...
			CprkActivity mActivity=(CprkActivity) getActivity();
			if(mActivity.getEntity()==null
					||mActivity.getEntity().getEntry()==null
					||mActivity.getEntity().getEntry().get(index)==null){
				return false;
			}
			theEntry=(JrPdaCprkbillentry) mActivity.getEntity().getEntry().get(index).clone();
...
//	}
	...
	protected void intiEntry(){
		this.entrystatus=0;
		//this._isqycw="0";
		theEntry=new JrPdaCprkbillentry();
...
//		mActivity.setMat_hasQtyMap(mat_hasQtyMap);
	...
	public boolean decodeMat(TextView v, int actionId, KeyEvent event){		
		...
		if(!checker.srcbillCheck(mActivity.getEntity().getBill().getFcpbzdjNo())){
			return false;
		}
		...
		if("B2".equals(_ftype)){
			...
			ftm.setText("");
			fqty.setText("");
			mActivity.getPresenter().decodeThBarcode(this, _barcode,mActivity.getEntity().getBill().getFcpbzdjNo());
		}
		else{
...
//		super.decodeBaseInfoInner(_barcode, Constance.requestCode_baseinfoMaterial);
	...
	public boolean decodeMatByCode(String ftype,String catchDecode) {
		// TODO Auto-generated method stub
		if(!checker.srcbillCheck(mActivity.getEntity().getBill().getFcpbzdjNo())){
			return false;
		}
		if("B2".equals(ftype)){
			...
			if(StringUtils.isEmpty(theEntry.getFstockName())){
				...
				if("0".equals(_isqycw)){
					...
					mActivity.getPresenter().decodeSpBarcode(this, catchDecode,theEntry.getFstockNumber());
				}			
			}		
		}
		else if("D2".equals(ftype)){
			for(int i=0;i<mActivity.getEntity().getEntry().size();i++){
				if(catchDecode.equals(mActivity.getEntity().getEntry().get(i).getFdytm())){
					alertDialog.setMessage("该条码已扫描过，不允许重复扫描！").show();
					new Thread(new Runnable() {
			            @Override
			            public void run() {
			...
			ftm.setText("");
			fqty.setText("");
			mActivity.getPresenter().decodeThBarcode(this, catchDecode,mActivity.getEntity().getBill().getFcpbzdjNo());
		}
		else{
			alertDialog.setMessage(Constance.check_SrcBillError).show();
		}
